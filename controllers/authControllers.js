...
module.exports.signupWorker = async (req, res) => {
    ...
    try{
        ...
        if (existingUser === null) {
            const id = v4();
            const otp = await Otp.findOne({ where: { mobileNumber }});
            if(otp === null || otp.isUsed){
                console.log("code does not exist");
                res.status(400).json({message: "otp code does not exist", error: "user signup failed."});
            } else {
                if(otp.otp !== otpCode){
                    console.log("invalid code");
                    res.status(400).json({message: "invalid code", error: "user signup failed."});
                }
                await otp.update({ isUsed: true });
                const newUser = await User.create({ id, role: "worker", mobileNumber});
                if(newUser){
            ...
            res.status(404).json({error: "mobile number already exists"});
        }
    } catch(e){
...
module.exports.sendOtp = async(req, res) => {
    ...
    try{
        ...
        console.log(genOtp);
        const existingOtp = await Otp.findOne({ where: { mobileNumber } });
        if(existingOtp){
            await Otp.destroy({
                where: {
                    mobileNumber
        ...
        const otp = await Otp.create({mobileNumber, otp: genOtp});
        const params = {
    ...
    try{
        ...
        if (user === null) {
            ...
            if(user.role === "worker"){
                ...
                if(otp === null || otp.isUsed){
                    console.log("code does not exist");
                    res.status(400).json({message: "otp code does not exist", error: "user login failed."});
                } else {
                    if(otp.otp !== otpCode){
                        console.log("invalid code");
                        res.status(400).json({message: "invalid code", error: "user login failed."});
                    }
                    await otp.update({ isUsed: true });
                    const jwtToken =  jwt.sign(
                        {
